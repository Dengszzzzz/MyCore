...
public class DragPhotoActivity2 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mLocationList.addAll((Collection<? extends LocationBean>) getIntent().getSerializableExtra("locationList"));
        mList.addAll(getIntent().getStringArrayListExtra("datas"));
        mPhotoViews = new DragPhotoView[mList.size()];

        for (int i = 0; i < mPhotoViews.length; i++) {
            mPhotoViews[i] = (DragPhotoView) View.inflate(this, R.layout.item_drag_photo_viewpager, null);
            GlideUtils.loadImg(this,mPhotoViews[i],mList.get(i));
            //mPhotoViews[i].setImageResource(R.mipmap.wugeng);
            mPhotoViews[i].setOnTapListener(new DragPhotoView.OnTapListener() {
                @Override
                public void onTap(DragPhotoView view) {
                    finishWithAnimation();  //点击退出执行
                }
            });

            mPhotoViews[i].setTag(mList.get(i));
        ...
        mViewPager.getViewTreeObserver()
                .addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
                    ...
                    public void onGlobalLayout() {
                        ...
                        mOriginLeft = mLocationList.get(curPosition).getLeft();
                        mOriginTop = mLocationList.get(curPosition).getTop();
                        mOriginHeight = mLocationList.get(curPosition).getHeight();
                        mOriginWidth = mLocationList.get(curPosition).getWidth();
                        mOriginCenterX = mOriginLeft + mOriginWidth / 2;
                        mOriginCenterY = mOriginTop + mOriginHeight / 2;

                        int[] location = new int[2];

                        final DragPhotoView photoView = mPhotoViews[curPosition];
                        photoView.getLocationOnScreen(location);

                        mTargetHeight = (float) photoView.getHeight();  //目标图的宽高，即从上个界面的小图 -> 现界面大图
    ...
    private void performExitAnimation(final DragPhotoView view, float x, float y, float w, float h) {
        ...
        float viewX = mTargetWidth / 2 +   - mTargetWidth * mScaleX / 2;
    ...
    private void performEnterAnimation() {
        final DragPhotoView photoView = mPhotoViews[curPosition];
        ValueAnimator translateXAnimator = ValueAnimator.ofFloat(photoView.getX(), 0);  //左上点X值，从getX() -> 0
        translateXAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                photoView.setX((Float) valueAnimator.getAnimatedValue());
        ...
        ValueAnimator translateYAnimator = ValueAnimator.ofFloat(photoView.getY(), 0); //左上点y值，从getY() -> 0
        translateYAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                photoView.setY((Float) valueAnimator.getAnimatedValue());
        ...
        ValueAnimator scaleYAnimator = ValueAnimator.ofFloat(mScaleY, 1);  //缩小状态，从scaleY -> 1 回复正常
        scaleYAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                photoView.setScaleY((Float) valueAnimator.getAnimatedValue());
        ...
        ValueAnimator scaleXAnimator = ValueAnimator.ofFloat(mScaleX, 1);  //缩小状态，从scaleX -> 1 回复正常
        scaleXAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                photoView.setScaleX((Float) valueAnimator.getAnimatedValue());
