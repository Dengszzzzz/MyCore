...
public class BezierMagicCircleView extends View{

    private String TAG = "BezierMagicCircleView";
    ...
    //从上边开始，顺时针 p1->p2->p3->p4

    public BezierMagicCircleView(Context context) {
        this(context,null);
    }

    public BezierMagicCircleView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        initView();
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        radius = 50;
        c = radius*blackMagic;
        stretchDistance = radius;  //延伸的间距
        moveDistance = radius*(3/5f);  //移动距离
        cDistance = c*0.45f;  //造圆贝塞尔曲线C值
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        mPath.reset();
        canvas.translate(radius, radius);  //坐标轴移动到(radius,radius) 目的是画出完整圆
     ...
     */
    class VPoint{
        public float x;  //数据点x坐标
        public float y;  //数据点y坐标
